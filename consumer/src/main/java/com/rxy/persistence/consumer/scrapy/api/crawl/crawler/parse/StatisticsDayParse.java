 ...
 */
@Component
public class StatisticsDayParse extends AbstractParseArray<RpLoanStatisticsDay> {
    ...
    public void parseArray(Page page, JSONArray jsonArray) {
        ...
        if (jsonArray != null) {
            // 容器存储解析之后数据结果
            List<RpLoanStatisticsDay> dataList = new ArrayList<RpLoanStatisticsDay>();

            // 遍历array数组 json转java
            jsonArray.forEach(json -> {
                RpLoanStatisticsDay data = JSON.parseObject(json.toString(), RpLoanStatisticsDay.class);
                dataList.add(data);
            });

            // put到page pipeline中get之后持久化 以tableName字符串做为key
            page.putField(TableNameEnum.STATISTICSDAY.getName(), dataList);
        }
    }
}
